...
object ThrottleControl extends ThrottleControl{
  ...
  private val flexThrottle = throttle(3 msgsPer 1.second) //slowly does it with Flex or we will flood the R2 queues with events
  private val r2Throttle = throttle(12 msgsPer 1.second)   //r2 should be a bit quicker than flex: avoid too much time half migrated
